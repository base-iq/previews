# Titanium Cutting Board Bridge Page (Value variant) — Implementation Spec

**Brand:** BaseLocal
**Feature slug:** `titanium-cutting-board-value`
**Prototype:** [https://base-iq.github.io/previews/baselocal/prototypes/titanium-cutting-board-value/](https://base-iq.github.io/previews/baselocal/prototypes/titanium-cutting-board-value/)
**A/B test:** Value variant of `titanium-cutting-board` (the control). Same top-of-funnel (scorecard + rounds); adds a price/value layer before the recommendation to fix the conversion drop (clicks were up vs. the original control, conversions down — price objection at the offer page).

> **⚠️ A/B attribution — must do before launch:** this variant currently shares the control's exact tracking URL. For the test to be measurable, give this variant a **distinct `sub_id`** (or `utm_term`) on every CTA so conversions can be attributed to the value page vs. the control. The `sub_id=` param in the URL is empty right now.
...
**Value layer (this variant's reason for existing).** After the rounds, before the recommendation, a price-reframe block addresses the "$59.99 is a lot for a cutting board" objection three ways: (1) a **Price Comparison** — buying the four tools TIBO replaces separately runs ~$97 vs. $59.99; (2) **durability/buy-once** — plastic and wood get rebought (USDA "discard when grooved"; wood warps/cracks), titanium doesn't, so a Stat Callout frames the 10-year cost; (3) a prominent green **guarantee block** (30-day money-back + 1-year warranty) for risk reversal. All cost/durability figures are **illustrative and marked "confirm before launch"** until real numbers are supplied. The Price-Comparison and guarantee blocks compose existing tokens; the **guarantee block is a candidate to formalize via `extend-design-system`** if reused.
...
1. **Big-numeral header** — oversized terracotta numeral + H2 (no eyebrow). The numeral matches the numbered scorecard row that links here (`id="round-N"`); the H2 carries the round's focus on its own now that the eyebrow is gone.
...
| Round transition (each round) | Oversized Bitter terracotta numeral (64px mobile / 76px desktop) left of the H2 (eyebrow removed). H2s carry the focus standalone: R1 "A plastic board sheds plastic with every slice" / R2 "Plastic and wood trap what you can't wash out" / R3 "Raw chicken, then tomatoes: the swap nobody makes mid-recipe" / R4 "The dishwasher round" / R5 "A metal board that's softer than your blade" / R6 "The only board that sharpens the knife it meets". |
| Scoresheet → rounds connection | Each scorecard row label is a jump link (`.fc-row-link` → `#round-N`) prefixed with a terracotta numbered badge (1–6) matching the round numeral, with a trailing "›". Labels stay espresso; badge + chevron are the terracotta affordance; row hover tints cream. `html { scroll-behavior: smooth }` (auto under reduced-motion); round sections get `scroll-margin-top: 18px`. Turns the scoresheet into a clickable table of contents to each round's evidence. |
