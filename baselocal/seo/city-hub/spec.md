...
**Masthead composition (decided in iteration):** every fact renders exactly once — the reader count lives only in the Stat Grid cell, the update cadence only in the provenance line, and the entity only in breadcrumb/title/schema. The earlier draft's Social Proof Strip (byline + chip) was removed as pure duplication of both. Order: h1 → lede → hero photo with the Stat Grid pulled up over its bottom edge (`.stat-grid.overlap` — the photo and the numbers read as one unit, the city quantified) → one provenance line (`By the BaseLocal team · ● Checked this morning, before the 6 AM send.`) → primary subscribe. When the location has no photo, the grid follows the lede directly with no overlap (see `sparse.html`).
...
| Subscriber count `{N}` (meta, stat cell, CTAs) | engaged-audience data per location | No | **Count-display rule:** ≥ threshold → show, rounded (nearest 100 below 10k, nearest 1k above). **Below threshold or market < 90 days old → launch mode:** the count never renders anywhere; the provenance line leads with "● Just launched", subscribe copy becomes "be one of the first" early-adopter framing, meta uses the launch variant (§ 3), the subscriber stat cell drops. A small number is worse social proof than newness. Missing data entirely → same as launch mode minus the "just launched" claim |
| Location hero image | per-location CDN asset (`locations/{id}/website-*.png`, 1400×600 — same asset the current hub renders) | No | Missing → no banner (h1 leads the page, as `sparse.html` shows); og:image and CollectionPage `image` fall back to the brand logo. Rendered with explicit `width`/`height` and `fetchpriority="high"` — above the fold, never lazy |
| Most-read stories (top 5; minimum 3 to render): headline, one-paragraph blurb, source URL + name | newsletter click data, last 30 days (`analytics_link_performance`), **local/news stories only — affiliate and sponsored items never appear here** | No | Fewer than 3 qualifying stories (e.g. new market with no click history) → section omitted entirely, as `sparse.html` shows |
| Weekly stat cells (events this week, free things to do, new spots) | computed from events + business pipelines | No | Grid is 1×N — drop any cell whose count is unavailable or zero (see sparse: "new spots" cell removed); never render a `0` from a query error |
...
- **No noindex floor.** The city hub never noindexes on thinness (contrast the events hub, which noindexes 3–4 events and 404s below 3). A thin market still deserves to answer the subscribe query. `sparse.html` demonstrates this — Shelton renders fully indexable at 6 events and no spotlights.
...
- **Normal** (`index.html`): full stack — location hero, 4-cell stat grid, 4 curated picks, 3 new-spot cards, live directory module, 5 most-read stories, full Explore grid, two subscribe blocks.
- **Sparse + launch** (`sparse.html`, Shelton WA — a thin *and newly launched* market): no hero image yet; stat grid drops the "new spots" **and** subscriber cells; the picks collapse from four to two, with the section sub-line owning the quiet week; "New in {City}" and "Most read" are omitted (no spotlights, no click history); the directory module falls back to its teaser state (directory not yet populated for a brand-new market); the Explore section leans on nearby markets; **launch mode** replaces every subscriber count with "just launched / be one of the first" framing. Crucially, **no noindex** — the page stays a full, indexable answer to `shelton newsletter`.
