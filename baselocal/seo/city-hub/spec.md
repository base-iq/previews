...
Landmarks: `header` (site header — logo, Locations, Advertise) → **location bar** (sticky; section nav in the label register + subscribe CTA) → `main`, opening with the breadcrumb trail above the h1 → `footer` (site footer). Heading outline (one h1, no skips; day labels, stat labels, and the freshness stamp are **not** headings):
...
h2  {City}'s best businesses         → (promo module: value line, category chips, link to /best/)
...
**Directory module.** Promotes the business directory (see the `business-spotlight` prototype spec — per-location, editor-curated, paid pins always disclosed), which ships before or with this template. **Live state (default, shown on `index.html`):** one value line, the 8 fixed category chips (Eat & drink / Shop / Services / Wellness / Beauty / Fitness / Home & garden / Kids & pets — same taxonomy as the ad flow; decorative until category URLs exist, see Open questions), and a "Browse {City}'s best →" link to the location's `/best/` URL. **Fallback state (spec-only):** where a location's directory isn't yet live or populated, the link is replaced by a "readers hear first" line anchoring to `#subscribe` — nothing on the page may link to a URL that would 404.

**Location bar (shell — revised on the directory build, adopted here).** One band under the site header: section links in the label register (Lato 900 uppercase, tracked — deliberately distinct from the breadcrumb) — **Events · This weekend · Businesses · Latest edition**, live surfaces only — with the **"Get the newsletter" CTA** at the right. The **breadcrumb trail lives in the content flow above the h1**, not in the bar (crumbs in the bar read as a second menu). The CTA lives here, not in the site header, because subscribing is a per-location action; the sitewide header stays location-agnostic (logo · Locations · Advertise). On child surfaces the trail extends (`… › Events`) and the current section renders bold non-link (`aria-current="page"`). Links wrap on mobile — never horizontal scroll. **The bar is sticky on this template**: the page's action bar, keeping section navigation and the subscribe CTA one tap away through a ~5-screen scroll. On narrow viewports the links compact (11px) and share rows with the CTA; the breadcrumb, being in the content flow, simply scrolls with the page at its full trail. Production adds the design system's `.is-stuck` warm shadow (Sticky Filter & Utility Bar convention); in-page anchors carry `scroll-margin-top` to clear the pinned bar. **One-sticky-bar rule:** a template pins at most one bar — on the events hub the filter bar keeps the sticky slot and this location bar stays static there. This shell is a cross-template requirement: the events hub and directory adopt the same bar (supersedes their standalone breadcrumb rows) — coordinate via the publishing-infrastructure internal-linking requirement (strategy item 7). the h1 is `The {City} newsletter` — sentence case, aligned to the head of the target query. The city/state entity is carried by the breadcrumb immediately above, the title, and the schema; there is no kicker (a `{City}, {ST}` eyebrow duplicated the breadcrumb verbatim and was removed in iteration). A bare `{City}, {ST}` h1 was likewise rejected: it reads as a directory label and answers the subscribe-intent query with a map pin.
...
**Masthead composition (decided in iteration):** every fact renders exactly once — the update cadence only in the provenance line, the entity only in breadcrumb/title/schema. Order: h1 → lede → hero photo with the Stat Grid pulled up over its bottom edge (`.stat-grid.overlap` — the photo and the numbers read as one unit, the city quantified) → the provenance line (`By the BaseLocal team · ● Checked this morning, before the 6 AM send.`) → primary subscribe. **The Stat Grid is pure content proof** (events this week · free things to do · new spots · new stories — all freshness proof: what the newsletter produced for the city just now). **Audience proof lives only in the reader pill inside the subscribe card** (`● {N} {City} readers`, centered between the card's heading and form — social proof at the point of decision). When the location has no photo, the grid follows the lede directly with no overlap.
...
| Stat cells (events this week, free things to do, new spots, new stories) | events + business pipelines; **new stories = stories published for the location in the last 7 days, from the stories/newsletter DB (confirm window)** | No | Grid is 1×N — drop any cell whose count is unavailable or zero; never render a `0` from a query error |
...
- **Normal** (`index.html`): full stack — location hero, 4-cell stat grid (47 events · 18 free · 12 new spots · 37 new stories), subscribe card with reader pill, 4 curated picks, 3 new-spot cards, live directory module, 5 most-read stories, full Explore grid, two subscribe blocks.
...
8. ~~Directory URL pattern~~ — **Resolved on the directory build:** `/{state}/{city}/best/` with 8 category child URLs (see the directory spec). The module's chips can become real category links now that category URLs exist; the generator still needs the per-location live/populated flag for the fallback rule (§ 4).
