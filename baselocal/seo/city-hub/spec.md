...
**Masthead composition (decided in iteration):** every fact renders exactly once — the update cadence only in the provenance line, the entity only in breadcrumb/title/schema. Order: h1 → lede → hero photo with the Stat Grid pulled up over its bottom edge (`.stat-grid.overlap` — the photo and the numbers read as one unit, the city quantified) → the **proof row** (provenance line left — `By the BaseLocal team · ● Checked this morning, before the 6 AM send.` — and the **reader pill** right — `● {N} {City} readers`) → primary subscribe. **The Stat Grid is pure content proof** (events this week · free things to do · new spots · local stories — the lifetime story count is a citable depth number no clone network can match); audience proof lives only in the pill. When the location has no photo, the grid follows the lede directly with no overlap.
...
| Subscriber count `{N}` (meta + reader pill) | engaged-audience data per location | No | Renders as the proof-row pill `● {N} {City} readers`, rounded (nearest 100 below 10k, nearest 1k above). **Launch mode (market < ~90 days or below count threshold — confirm):** the pill reads `● Newly launched this month` instead of a small number, subscribe copy becomes "be one of the first", meta uses the launch variant (§ 3). All other stats still show their real numbers — new markets never hide data. Missing data entirely → pill omitted |
...
| Stat cells (events this week, free things to do, new spots, local stories) | events + business pipelines; **local stories = lifetime published story count for the location from the stories/newsletter DB** | No | Grid is 1×N — drop any cell whose count is unavailable or zero (see sparse: "new spots" cell removed); never render a `0` from a query error |
...
- **Normal** (`index.html`): full stack — location hero, 4-cell stat grid (47 events · 18 free · 12 new spots · 3,600 stories), reader pill, 4 curated picks, 3 new-spot cards, live directory module, 5 most-read stories, full Explore grid, two subscribe blocks.
- **Sparse + launch** (`sparse.html`, Shelton WA — a thin *and newly launched* market): hero renders as usual (prototype uses a stand-in CDN asset); stat grid shows its real numbers (6 events · 4 free · 140 stories; "new spots" dropped — none this month); the reader pill reads `● Newly launched this month`; the picks collapse from four to two, with the section sub-line owning the quiet week; "New in {City}" and "Most read" are omitted (no spotlights, no click history); the directory module falls back to its teaser state; the Explore section leans on nearby markets. Crucially, **no noindex** — the page stays a full, indexable answer to `shelton newsletter`.
